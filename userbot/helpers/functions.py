...
async def admin_groups(blackpanther):
    blackpanthergroups = []
    async for dialog in blackpanther.client.iter_dialogs():
        ...
        if (
            ...
            blackpanthergroups.append(entity.id)
    return blackpanthergroups
...
async def make_gif(event, file):
    ...
    async with event.client.conversation(chat) as conv:
        try:
            ...
            await event.client.send_read_acknowledge(conv.chat_id)
            blackpantherfile = await event.client.download_media(blackpantherresponse, "./temp")
            return await unzip(blackpantherfile)
...
async def iphonex(text):
    ...
    danish_baba = r.get("message")
    blackpantherurl = url(danish_baba)
    if not blackpantherurl:
        return "check syntax once more"
    with open("temp.png", "wb") as f:
        f.write(requests.get(danish_baba).content)
...
async def waifutxt(text, chat_id, reply_to_id, bot, borg):
    ...
    blackpanther = await sticcers[0].click("me", hide_via=True)
    if blackpanther:
        await bot.send_file(int(chat_id), blackpanther, reply_to=reply_to_id)
