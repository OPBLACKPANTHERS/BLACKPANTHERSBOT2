# Ascii module by @kraken_the_badass for @blackpanther_official
# A over powerful bot
# I know u will kang...
# GTFO!! MOTHERFUCKER!!!!!!!!!!!
...
async def _(event):
    ...
    if reply_message.sender.bot:
        ...
        return
    danish_baba = await edit_or_reply(event, "Wait making ASCII...🤓🔥🔥")
    async with event.client.conversation(chat) as conv:
        try:
            ...
            await danish_baba.edit("`Please unblock @asciiart_bot and try again`")
            return
        if response.text.startswith("Forward"):
            await danish_baba.edit(
...
async def _(event):
    ...
    if reply_message.sender.bot:
        await edit_or_reply(event, "Reply to actual users message.😒🤐")
        return
    danish_baba = await edit_or_reply(event, "`Processing`")
    async with event.client.conversation(chat) as conv:
        try:
            ...
            await danish_baba.edit("Please unblock @Lines50Bot and try again")
            return
        await danish_baba.delete()
        await event.client.send_file(
            event.chat_id,
