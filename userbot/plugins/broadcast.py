...
async def blackpantherbroadcast_send(event):
    if event.fwd_from:
        return
    blackpantherinput_str = event.pattern_match.group(1)
    if not blackpantherinput_str:
    ...
    keyword = blackpantherinput_str.lower()
    no_of_chats = sql.num_broadcastlist_chat(keyword)
    group_ = Get(blackpanther)
    if no_of_chats == 0:
        return await edit_delete(
            event,
    ...
    blackpantherevent = await edit_or_reply(
        event,
    ...
    await blackpantherevent.edit(resultext)
    if BOTLOG:
...
async def blackpantherbroadcast_add(event):
    if event.fwd_from:
        return
    blackpantherinput_str = event.pattern_match.group(1)
    if not blackpantherinput_str:
...
async def blackpantherbroadcast_remove(event):
    if event.fwd_from:
        return
    blackpantherinput_str = event.pattern_match.group(1)
    if not blackpantherinput_str:
...
async def blackpantherbroadcast_list(event):
    if event.fwd_from:
        return
    blackpantherinput_str = event.pattern_match.group(1)
    if not blackpantherinput_str:
    ...
    await edit_or_reply(blackpantherevent, finaloutput)
...
async def blackpantherbroadcast_remove(event):
    ...
    args = blackpantherinput_str.split(" ")
    if len(args) != 2:
        return await edit_delete(
            event,
...
async def blackpantherbroadcast_delete(event):
    if event.fwd_from:
        return
    blackpantherinput_str = event.pattern_match.group(1)
    check1 = sql.num_broadcastlist_chat(blackpantherinput_str)
    if check1 < 1:
        return await edit_delete(
            event,
            f"Are you sure that there is category {blackpantherinput_str}",
            parse_mode=parse_pre,
        )
    try:
        sql.del_keyword_broadcastlist(blackpantherinput_str)
        await edit_or_reply(
            event,
            f"Successfully deleted the category {blackpantherinput_str}",
            parse_mode=parse_pre,
        )
    except Exception as e:
        await edit_delete(
