...
from blackpantherbot.utils import admin_cmd, edit_or_reply, progress, sudo_cmd
...
async def _(blackpanther):
    if blackpanther.fwd_from:
        return
    reply_to_id = blackpanther.message.id
    if blackpanther.reply_to_msg_id:
        reply_to_id = blackpanther.reply_to_msg_id
    event = await edit_or_reply(blackpanther, "Converting.....")
    ...
    if event.reply_to_msg_id:
        ...
        downloaded_file_name = await blackpanther.client.download_media(
            reply_message, downloaded_file_name
        )
        if os.path.exists(downloaded_file_name):
            caat = await blackpanther.client.send_file(
                event.chat_id,
...
async def _(event):
    if event.fwd_from:
        return
    blackpantherreply = await event.get_reply_message()
    if not blackpantherreply or not blackpantherreply.media or not blackpantherreply.media.document:
        return await edit_or_reply(event, "`Stupid!, This is not animated sticker.`")
    if blackpantherreply.media.document.mime_type != "application/x-tgsticker":
        return await edit_or_reply(event, "`Stupid!, This is not animated sticker.`")
    reply_to_id = event.message
    if event.reply_to_msg_id:
        reply_to_id = await event.get_reply_message()
    chat = "@tgstogifbot"
    blackpantherevent = await edit_or_reply(event, "`Converting to gif ...`")
    async with event.client.conversation(chat) as conv:
        try:
            await silently_send_message(conv, "/start")
            await event.client.send_file(chat, blackpantherreply.media)
            response = await conv.get_response()
            await event.client.send_read_acknowledge(conv.chat_id)
            if response.text.startswith("Send me an animated sticker!"):
                return await blackpantherevent.edit("`This file is not supported`")
            blackpantherresponse = response if response.media else await conv.get_response()
            await event.client.send_read_acknowledge(conv.chat_id)
            blackpantherfile = Path(await event.client.download_media(blackpantherresponse, "./temp/"))
            blackpanthergif = Path(await unzip(blackpantherfile))
            ...
            for files in (blackpanthergif, blackpantherfile):
                if files and os.path.exists(files):
                    os.remove(files)
        except YouBlockedUserError:
            await blackpantherevent.edit("Unblock @tgstogifbot")
            return
