...
async def all_groups_id(blackpanther):
    blackpanthergroups = []
...
async def _(event):
    ...
    if Config.BLACKPANTHERBOT_LOGGER is None:
        await edit_or_reply(
            event,
            "Please set the required environment variable `BLACKPANTHERBOT_LOGGER` for this plugin to work",
        )
        return
    try:
        e = await event.client.get_entity(Config.BLACKPANTHERBOT_LOGGER)
