...
async def download(event):
    ...
    if Var.GIT_REPO_NAME is None:
        await edit_or_reply(event, "`Please ADD Proper Github Repo Name of blackpantherBot`")
        return
    blackpantherbot = await edit_or_reply(event, "Processing ...")
    ...
    try:
        ...
        await blackpantherbot.edit(str(e))
    else:
        end = datetime.now()
        ms = (end - start).seconds
        await event.delete()
        await blackpantherbot.edit(
            "Downloaded to `{}` in {} seconds.".format(downloaded_file_name, ms)
        )
        await blackpantherbot.edit("Committing to Github....")
        await git_commit(downloaded_file_name, blackpantherbot)


async def git_commit(file_name, blackpantherbot):
    content_list = []
    ...
    for i in content_list:
        ...
        if i == 'ContentFile(path="' + file_name + '")':
            return await blackpantherbot.edit("`File Already Exists`")
            create_file = False
    ...
    if create_file == True:
        ...
        try:
            ...
            await blackpantherbot.edit("Cannot Upload Plugin")
    else:
        return await blackpantherbot.edit("`Committed Suicide`")
