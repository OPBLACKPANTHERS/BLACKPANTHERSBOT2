                                 ...
                                 ChatAdminRights,
                                   ChatBannedRights,
                                     MessageEntityMentionName,
                                       MessageMediaPhoto)
from blackpantherbot.utils import register, errors_handler
...
async def gben(userbot):
    mb = blackpanther = userbot
    i = 0
    sender = await mb.get_sender()
    me = await userbot.client.get_me()
    await blackpanther.edit("`promoting...`")
    ...
    try:
        ...
        pass
    if me == user:
       k = await blackpanther.edit("U want to promote urself 😑😑 waao..")
       return
    try:
        if not rank:
            rank = "ㅤㅤ"
    except:
        return await blackpanther.edit(f"**Something W3NT Wrong 🤔**")
    if user:
        ...
        for x in telchanel:
          try:
             ...
             i += 1
             await blackpanther.edit(f"**Promoted in Chats **: `{i}`")
          except:
             pass
    else:
        await blackpanther.edit(f"**Reply to a user you dumbo !!**")
    return await blackpanther.edit(
...
async def gben(userbot):
    ...
    sender = await mb.get_sender()
    me = await userbot.client.get_me()
    await blackpanther.edit("`demoting...`")
    my_mention = "[{}](tg://user?id={})".format(me.first_name, me.id)
    f"@{me.username}" if me.username else my_mention
    await userbot.get_chat()
    if userbot.is_private:
    ...
    try:
        ...
        pass
    if me == user:
       k = await blackpanther.edit("U want to demote urself 😑😑 waao..")
       return
    try:
        if not rank:
            rank = "ㅤㅤ"
    ...
    if user:
        ...
        for x in telchanel:
          try:
             await userbot.client(EditAdminRequest(x, user, rgt, rank))
             i += 1
             await blackpanther.edit(f"**Demoted in Chats **: `{i}`")
          except:
             pass
    else:
        await blackpanther.edit(f"**Reply to a user you dumbo !!**")
