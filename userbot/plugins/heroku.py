...
from userbot.helpers.extras import delete_blackpanther, edit_or_reply as eor
...
BLACKPANTHER_NAME = str(ALIVE_NAME) if ALIVE_NAME else "BlackPanther User"
danish_baba = bot.uid
...
async def _(dyno):
    if (HEROKU_APP_NAME is None) or (HEROKU_API_KEY is None):  
        return await eor(dyno, f"Make Sure Your HEROKU_APP_NAME & HEROKU_API_KEY are filled correct. Visit @BlackPantherBot_Support for help.", link_preview=False)
    try:
        Heroku = heroku3.from_key(HEROKU_API_KEY)
        app = Heroku.app(HEROKU_APP_NAME)        
    except BaseException:
        return await dyno.reply(f"Make Sure Your Heroku App Name & API Key are filled correct. Visit @BlackPantherBot_Support for help.", link_preview=False)
   # event = await eor(dyno, "Downloading Logs...")
    blackpanther_data = app.get_log()
    await eor(dyno, blackpanther_data, deflink=True, linktext=f"**🗒️ Heroku Logs of 💯 lines. 🗒️**\n\n🌟 **Bot Of :** [{BLACKPANTHER_NAME}](tg://user?id={danish_baba})\n\n🚀** Pasted**  ")
