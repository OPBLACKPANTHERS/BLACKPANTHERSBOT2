...
# porting to blackpanther by @kraken_the_badass...
...
async def get_users(event):
    ...
    if not sender.id == me.id:
        blackpanther = await edit_or_reply(event, "`processing...`")
    else:
        blackpanther = await edit_or_reply(event, "`processing...`")
    danish_baba = await get_chatinfo(event)
    chat = await event.get_chat()
    if event.is_private:
        return await blackpanther.edit("`Sorry, Cant add users here`")
    s = 0
    f = 0
    error = "None"

    await blackpanther.edit("**TerminalStatus**\n\n`Collecting Users.......`")
    async for user in event.client.iter_participants(danish_baba.full_chat.id):
        try:
