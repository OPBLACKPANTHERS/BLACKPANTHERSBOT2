...
#modifiy by @D3krish for BlackPantherbot
#If you kang this plugin please don't forget to give credit to BlackPantherbot and Hellbot. 
import os
import random
import time
...
async def _(blackpantherevent):
    event = await edit_or_reply(blackpantherevent, "`Processing.....`")
    fnt = await get_font_file(blackpantherevent.client, "@D3VIL_FONTSS")
    if blackpantherevent.reply_to_msg_id:
        rply = await blackpantherevent.get_reply_message()
    ...
    text = blackpantherevent.pattern_match.group(1)
    if len(text) <= 8:
        font_size_ = 150
        strik = 10
    elif len(text) >= 9:
    ...
    file_name = "BlackPantherBot.png"
    img.save(file_name, "png")
    await bot.send_file(
        blackpantherevent.chat_id,
        file_name,
        caption=f"**мα∂ε вү  :** 『[{DEFAULTUSER}](tg://user?id={blackpanther})』\n\n",
    )
    await event.delete()
    try:
        os.remove(file_name)
