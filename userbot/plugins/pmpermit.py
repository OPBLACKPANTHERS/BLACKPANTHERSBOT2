...
from userbot import ALIVE_NAME, CUSTOM_PMPERMIT, BLACKPANTHER_ID
...
USER_BOT_WARN_ZERO = "**You were spamming my sweet master's inbox, henceforth you have been blocked by my master's BlackPantherBot.**\n__Now GTFO, i'm busy__"
USER_BOT_NO_WARN = (
    "Hello, This is **BlackPantherBot Ultra Private Security Protocol⚠️**.\n"
    f"This is my master {DEFAULTUSER}'s Inbox\n"
    f"\n**{danish_baba}**\n\n"
    "To start a valid conversation\n🔱Register Your Request!🔱\nSend `/start` To Register Your Request\nHopefully u will get a reply🔥"
)

if Var.BLACKPANTHERBOT_LOGGER is not None:
        ...
        if len(APPROVED_PMs) > 4095:
            with io.BytesIO(str.encode(APPROVED_PMs)) as out_file:
                ...
                await event.client.send_file(
                    ...
                    force_document=True,
                    allow_cache=False,
                    caption="[BlackPantherBot]Current Approved PMs",
                    reply_to=event,
                )
                await event.delete()
        else:
    ...
    async def on_new_private_message(event):
        ...
        if Var.BLACKPANTHERBOT_LOGGER is None:
            return
    ...
    async def do_pm_permit_action(chat_id, event):
        ...
        if PM_WARNS[chat_id] == Config.MAX_FLOOD_IN_P_M_s:
            ...
            try:
                await event.client.send_message(
                    entity=Var.BLACKPANTHERBOT_LOGGER,
                    message=the_message,
                    # reply_to=,
                    # parse_mode="html",
                    link_preview=False,
        ...
        r = await bot.send_file(
            event.chat_id, BLACKPANTHERPIC, caption=USER_BOT_NO_WARN, force_document=False
        )
        PM_WARNS[chat_id] += 1
        if chat_id in PREV_REPLY_MESSAGE:
            await PREV_REPLY_MESSAGE[chat_id].delete()
