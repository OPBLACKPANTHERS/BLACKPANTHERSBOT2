...
from blackpantherbot.utils import admin_cmd, sudo_cmd, edit_or_reply, progress
...
async def nope(danish_baba):
    ...
    troll = await bot.inline_query("iLyricsBot", f"{(deEmojify(blackpanther))}")
    ...
    async with borg.conversation("@DeezLoadBot") as conv:
        try:
            ...
            await borg.send_file(
                event.chat_id,
                songh,
                caption="🔆**Here's the requested song!**🔆\n`Check out` [BlackPantherBot](https://t.me/BlackPantherBot_Chit_Chat)",
            )
            await event.delete()
        except YouBlockedUserError:
