...
async def bigspam(blackpanther):
    if not blackpanther.text[0].isalpha() and blackpanther.text[0] not in ("/", "#", "@", "!"):
        blackpanther_msg = blackpanther.text
        blackpantherbot_count = int(blackpanther_msg[9:13])
        blackpanther_spam = str(blackpanther.text[13:])
        for i in range(1, blackpantherbot_count):
            await blackpanther.respond(blackpanther_spam)
        await blackpanther.delete()
        if LOGGER:
            await blackpanther.client.send_message(
...
CmdHelp("spam").add_command(
  ...
  "bigspam", "<count> <text>", "Sends the text 'X' number of times. This what blackpantherbot iz known for. The Best BigSpam Ever", ".bigspam 5000 Hello"
).add()
