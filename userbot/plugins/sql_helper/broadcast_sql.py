...
class BlackPantherBroadcast(BASE):
    __tablename__ = "blackpantherbroadcast"
    ...
    def __repr__(self):
        return "<BlackPanther Broadcast channels '%s' for %s>" % (self.group_id, self.keywoard)

    def __eq__(self, other):
        return bool(
            isinstance(other, BlackPantherBroadcast)
            and self.keywoard == other.keywoard
            and self.group_id == other.group_id
        )


BlackPantherBroadcast.__table__.create(checkfirst=True)

BlackPantherBROADCAST_INSERTION_LOCK = threading.RLock()
...
def add_to_broadcastlist(keywoard, group_id):
    with BlackPantherBROADCAST_INSERTION_LOCK:
        broadcast_group = BlackPantherBroadcast(keywoard, str(group_id))
...
def rm_from_broadcastlist(keywoard, group_id):
    with BlackPantherBROADCAST_INSERTION_LOCK:
        broadcast_group = SESSION.query(BlackPantherBroadcast).get((keywoard, str(group_id)))
        if broadcast_group:
...
def del_keyword_broadcastlist(keywoard):
    with BlackPantherBROADCAST_INSERTION_LOCK:
        broadcast_group = (
            SESSION.query(BlackPantherBroadcast.keywoard)
            .filter(BlackPantherBroadcast.keywoard == keywoard)
            .delete()
        )
...
def get_broadcastlist_chats():
    try:
        chats = SESSION.query(BlackPantherBroadcast.keywoard).distinct().all()
...
def num_broadcastlist():
    try:
        return SESSION.query(BlackPantherBroadcast).count()
    finally:
        SESSION.close()
...
def num_broadcastlist_chats():
    try:
        return SESSION.query(func.count(distinct(BlackPantherBroadcast.keywoard))).scalar()
    finally:
        SESSION.close()
...
def __load_chat_broadcastlists():
    try:
        ...
        all_groups = SESSION.query(BlackPantherBroadcast).all()
        for x in all_groups:
