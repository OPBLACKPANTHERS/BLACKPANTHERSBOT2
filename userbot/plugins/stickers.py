...
blackpantherbot = Config.CUSTOM_STICKER_PACK_NAME
...
async def kang(args):
    ...
    if photo:
        ...
        packname = f"HB_{user.username}_BlackPantherBot_{pack}"
        packnick = (
            f"{blackpantherbot} Vol.{pack}"
            if blackpantherbot
            else f"@{user.username}'s BlackPantherBot Vol.{pack}"
        )
        cmd = "/newpack"
        file = io.BytesIO()
