...
async def _(event):
    ...
    blackpantherevent = await edit_or_reply(event, "Downloading to my local, for analysis  🙇")
    if event.reply_to_msg_id:
        ...
        if (
            ...
            await blackpantherevent.edit("Starting analysis, using IBM WatSon Speech To Text")
            headers = {
            ...
            if "results" in r:
                ...
                await blackpantherevent.edit(string_to_show)
            else:
                await blackpantherevent.edit(r["error"])
            # now, remove the temporary file
            os.remove(required_file_name)
    else:
        await blackpantherevent.edit("Reply to a voice message, to get the relevant transcript.")
