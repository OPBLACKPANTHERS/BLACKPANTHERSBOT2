...
if Config.TAG_LOGGER:
    ...
    async def all_messages_catcher(event):
        ...
        danish_baba = await event.client.get_entity(event.sender_id)
        if danish_baba.bot or danish_baba.verified or danish_baba.support:
            return

        danish_babam = f"[{get_display_name(danish_baba)}](tg://user?id={danish_baba.id})"
