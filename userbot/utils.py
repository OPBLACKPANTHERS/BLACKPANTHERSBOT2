...
def load_module(shortname):
    if shortname.startswith("__"):
        ...
        LOGS.info("BlackPantherBot - Successfully imported " + shortname)
    else:
        import userbot.utils
        ...
        mod.Config = Config
        mod.borg = bot
        mod.blackpantherbot = bot
        mod.edit_or_reply = edit_or_reply
        mod.delete_blackpanther = delete_blackpanther
        mod.media_type = media_type
        # support for blackpantherbot originals
        sys.modules["blackpantherbot.utils"] = userbot.utils
        sys.modules["blackpantherbot"] = userbot
    ...
    if pattern is not None:
        if pattern.startswith(r"\#"):
            ...
            if len(Config.COMMAND_HAND_LER) == 2:
                blackpantherreg = "^" + Config.COMMAND_HAND_LER
                reg = Config.COMMAND_HAND_LER[1]
            elif len(Config.COMMAND_HAND_LER) == 1:
                blackpantherreg = "^\\" + Config.COMMAND_HAND_LER
                reg = Config.COMMAND_HAND_LER
            args["pattern"] = re.compile(blackpantherreg + pattern)
    ...
    if pattern is not None:
        if pattern.startswith(r"\#"):
            ...
            if len(Config.SUDO_COMMAND_HAND_LER) == 2:
                blackpantherreg = "^" + Config.SUDO_COMMAND_HAND_LER
...
async def delete_blackpanther(event, text, time=None, parse_mode=None, link_preview=None):
    parse_mode = parse_mode or "md"
    link_preview = link_preview or False
    time = time or 5
    if event.sender_id in Config.SUDO_USERS:
        ...
        blackpantherevent = await event.edit(
            text, link_preview=link_preview, parse_mode=parse_mode
        )
    await asyncio.sleep(time)
    return await blackpantherevent.delete()
...
async def unsavegif(event, danish_baba):
    try:
        await event.client(
            functions.messages.SaveGifRequest(
                id=types.InputDocument(
                    id=danish_baba.media.document.id,
                    access_hash=danish_baba.media.document.access_hash,
                    file_reference=danish_baba.media.document.file_reference,
                ),
                unsave=True,
            )
        )
